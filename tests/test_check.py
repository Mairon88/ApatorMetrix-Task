...
import pytest
import subprocess
from constants import FAILURE_CODE, PATH_TO_SUB_CHECK_FILE, PYTHON_VER
...
def run_subprocess_check(params):
    ...
    result = subprocess.run(
        [PYTHON_VER, PATH_TO_SUB_CHECK_FILE, json_data],
        capture_output=True,
        text=True,
...
def test_check_correct_ip_in_added_prefix(base_ip, mask, ip_addr_to_check):
    ...
    if result.returncode:
        assert f"Process finished with exit code {result.returncode}" == str(mask)
...
def test_check_correct_ip_in_added_multi_prefix(multi_prefix, ip_addr_to_check, expected):
    ...
    result = run_subprocess_check(params)
    if result.returncode:
        assert f"Process finished with exit code {result.returncode}" == expected
    assert result.stdout.rstrip("\n") == expected
